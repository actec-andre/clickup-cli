...
def build_agent_info() -> dict[str, Any]:
    ...
    return {
        ...
        "namespace": {
            ...
            "IDS": "Dict with the workspace id. Space/list ids are resolved at runtime, not stored here.",
            "TEAM_ID": "The workspace id (override with the CLICKUP_TEAM_ID environment variable).",
            "spaces": "spaces() -> {name: id} for the workspace, resolved live. Use this instead of hard-coded space ids.",
            "json": "json module.",
            "os": "os module.",
        ...
        "patterns": {
            ...
            "discover_spaces": "result = spaces()  # {name: id}, resolved live — never hard-code a space id",
            "discover_lists": "result = cu.get_folderless_lists(spaces()['Magento'])['lists']  # -> list ids",
            "list_tasks": "result = list(cu.iter_tasks(list_id))  # get list_id from discover_lists first",
            "space_tasks": "result = list(cu.iter_space_tasks(spaces()['Marketing']))  # every task in a space",
            "resolve_people": "result = {m['email']: m['id'] for m in cu.get_members(TEAM_ID)}  # name/email -> assignee id",
            "create_task": "result = cu.create_task(list_id, name='New task', status='to do')",
